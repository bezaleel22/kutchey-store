import type { ActiveOrder$input, ActiveOrder$result, QueryStore, QueryStoreFetchParams} from '$houdini'

export declare class ActiveOrderStore extends QueryStore<ActiveOrder$result, ActiveOrder$input> {
	/**
	 * ### Route Loads
	 ...
	 * 	const store = new ActiveOrderStoreStore()
 ...
 *   };
 *
 *   return await load_ActiveOrder({ event, variables });
 * };
 * ```
 *
 ...
 *
 * ```js
 * import { loadAll, load_ActiveOrder } from '$houdini';
 * import type { PageLoad } from './$types';
 *
 * export const load: PageLoad = async (event) => {
 *   const variables = {
 ...
 *     load_ActiveOrder({ event, variables }),
 *     // load_ANOTHER_STORE
 *   );
 * };
 * ```
 */
export declare const load_ActiveOrder: (params: QueryStoreFetchParams<ActiveOrder$result, ActiveOrder$input>) => Promise<{ActiveOrder: ActiveOrderStore}>
