import type { Products$input, Products$result, QueryStore, QueryStoreFetchParams} from '$houdini'

export declare class ProductsStore extends QueryStore<Products$result, Products$input> {
	/**
	 * ### Route Loads
	 ...
	 * 	const store = new ProductsStoreStore()
 ...
 *
 * ```js
 * import { load_Products } from '$houdini';
 ...
 *   };
 *
 *   return await load_Products({ event, variables });
 * };
 * ```
 *
 ...
 *     load_Products({ event, variables }),
 *     // load_ANOTHER_STORE
 *   );
 * };
 * ```
 */
export declare const load_Products: (params: QueryStoreFetchParams<Products$result, Products$input>) => Promise<{Products: ProductsStore}>
