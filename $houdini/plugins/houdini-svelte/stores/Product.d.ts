import type { Product$input, Product$result, QueryStore, QueryStoreFetchParams} from '$houdini'

export declare class ProductStore extends QueryStore<Product$result, Product$input> {
	/**
	 * ### Route Loads
	 ...
	 * 	const store = new ProductStoreStore()
 ...
 *   };
 *
 *   return await load_Product({ event, variables });
 * };
 * ```
 *
 ...
 *     load_Product({ event, variables }),
 *     // load_ANOTHER_STORE
 *   );
 * };
 * ```
 */
export declare const load_Product: (params: QueryStoreFetchParams<Product$result, Product$input>) => Promise<{Product: ProductStore}>
